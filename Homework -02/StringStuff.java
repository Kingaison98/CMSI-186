...
public class StringStuff {
   ...
   */
   private static String vowelString = "aeiouyAEIOUY";
   private static char[] vowels = vowelString.toCharArray();
   private static String evenChars = "bdfhjlnprtvxzBDFHJLNPRTVXZ";
   private static char[] evens = evenChars.toCharArray();
   private static String oddChars = "acegikmoqsuwyACEGIKMOQSUWY";
   private static char[] odds = oddChars.toCharArray();
   ...
   */
   public static String evensOnlyNoDupes( String s ) {
      StringBuffer original = new StringBuffer(evensOnly(s));
      StringBuffer noDupe = original;
      for(int i = 0; i < original.length(); i++){
        for(int j = i + 1 ; j < noDupe.length() ; j++){
          if(noDupe.charAt(j) == original.charAt(i)) {
            //System.out.println("Comparing character " + i + " to " + j);
            noDupe.deleteCharAt(j);
            j -= 1;
          }
          //System.out.println("Compared " + original.charAt(i) + " to " + noDupe.charAt(j) + ". New string is " + noDupe);
        }
      }
      System.out.println(noDupe);
      return noDupe.toString();
   }
   ...
   */
   public static String oddsOnlyNoDupes( String s ) {
     StringBuffer original = new StringBuffer(oddsOnly(s));
     StringBuffer noDupe = original;
     for(int i = 0; i < original.length(); i++){
       for(int j = i + 1 ; j < noDupe.length(); j++) {
         if(noDupe.charAt(j) == original.charAt(i)) {
   ...
   */
   public static String reverse( String s ) {
     ...
     System.out.println("Reversed is " + flipped);
